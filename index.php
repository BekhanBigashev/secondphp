...
foreach ($result['result']['result']['get_users'] as $user) {
    foreach ($result['result']['result']['get_tasks']['tasks'] as $task) {
        if ($user['ID'] == $task['responsible']['id']){
            $userTasks[$task['id']] = $task['title'];//если id польователя равно id ответственного в задаче, то во временное хранилище загружаются все задачи для одного пользователя         
        }
    }
